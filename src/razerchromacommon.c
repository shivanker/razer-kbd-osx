 ...
 *
 * 0x02, 0x00 Will make M1-5 and FN emit normal keystrokes. Some sort of factory
 * test mode. Not recommended to be used.
 */
 ...
 * 00     3f    0000   00    03       03    00  010801                     | SET
 * LED STATE (VARSTR, GAMEMODE, ON) 00     3f    0000   00    03       03    00
 * 010800                     | SET LED STATE (VARSTR, GAMEMODE, OFF)
 */
struct razer_report razer_chroma_standard_set_led_state(
...
struct razer_report razer_chroma_standard_set_led_blinking(
    unsigned char variable_storage, unsigned char led_id) {
 ...
 */
struct razer_report razer_chroma_standard_get_led_state(
    unsigned char variable_storage, unsigned char led_id) {
 ...
 */
struct razer_report razer_chroma_standard_set_led_rgb(
    unsigned char variable_storage, unsigned char led_id,
 ...
 */
struct razer_report razer_chroma_standard_get_led_rgb(
    unsigned char variable_storage, unsigned char led_id) {
 ...
 * ? TODO fill this
 */
struct razer_report razer_chroma_standard_set_led_effect(
    unsigned char variable_storage, unsigned char led_id,
 ...
 * ? TODO fill this
 */
struct razer_report razer_chroma_standard_get_led_effect(
    unsigned char variable_storage, unsigned char led_id) {
 ...
 * ? TODO fill this
 */
struct razer_report razer_chroma_standard_set_led_brightness(
    unsigned char variable_storage, unsigned char led_id,
 ...
 * ? TODO fill this
 */
struct razer_report razer_chroma_standard_get_led_brightness(
    unsigned char variable_storage, unsigned char led_id) {
 ...
 * ? TODO fill this
 */
struct razer_report razer_chroma_standard_matrix_effect_none(
    unsigned char variable_storage, unsigned char led_id) {
 ...
 * ? TODO fill this
 */
struct razer_report razer_chroma_standard_matrix_effect_wave(
    unsigned char variable_storage, unsigned char led_id,
 ...
 * ? TODO fill this
 */
struct razer_report razer_chroma_standard_matrix_effect_spectrum(
    unsigned char variable_storage, unsigned char led_id) {
  struct razer_report report = get_razer_report(0x03, 0x0A, 0x01);
...
struct razer_report razer_chroma_standard_matrix_effect_reactive(
    unsigned char variable_storage, unsigned char led_id, unsigned char speed,
    struct razer_rgb *rgb1) {
 ...
 * ? TODO fill this
 */
struct razer_report razer_chroma_standard_matrix_effect_static(
    unsigned char variable_storage, unsigned char led_id,
    struct razer_rgb *rgb1) {
 ...
 * ??
 *
 * Apparently Ultimate2016, Stealth and Stealth2016 need frame id to be 0x00, I
 * don't think it's needed (depending on set_custom_frame)
 */
 ...
 *
 * This sets the colour of a row on the keyboard. Takes in an array of RGB
 * bytes. The mappings below are correct for the BlackWidow Chroma. The
 * BlackWidow Ultimate 2016 contains LEDs under the spacebar and the FN key so
 * there will be changes once I get the hardware.
 *
 * Row 0:
 *  0      Unused
 *  1      ESC
...
struct razer_report razer_chroma_standard_matrix_set_custom_frame(
    unsigned char row_index, unsigned char start_col, unsigned char stop_col,
    unsigned char *rgb_data) {
  size_t row_length = (size_t)(((stop_col + 1) - start_col) * 3);
  struct razer_report report = get_razer_report(
      0x03, 0x0B, 0x46);  // In theory should be able to leave data size at max
                          // as we have start/stop

  // printk(KERN_ALERT "razerkbd: Row ID: %d, Start: %d, Stop: %d, row length:
 ...
 *
 * Status Trans Packet Proto DataSize Class CMD Args
 * 00     3f    0000   00    06       0f    02  010500000000  | SET LED MATRIX
 * Effect (VARSTR, Backlight, None 0x00, 0x000000)
 */
struct razer_report razer_chroma_extended_matrix_effect_none(
    unsigned char variable_storage, unsigned char led_id) {
 ...
 *
 * Status Trans Packet Proto DataSize Class CMD Args
 * 00     3f    0000   00    09       0f    02  010501000001ff0000 | SET LED
 * MATRIX Effect (VARSTR, Backlight, Static 0x01, ? 0x000001, RGB 0xFF0000) 00
 * 3f    0000   00    09       0f    02  01050100000100ff00 | SET LED MATRIX
 * Effect (VARSTR, Backlight, Static 0x01, ? 0x000001, RGB 0x00FF00) 00     3f
 * 0000   00    09       0f    02  010501000001008000 | SET LED MATRIX Effect
 * (VARSTR, Backlight, Static 0x01, ? 0x000001, RGB 0x008000)
 */
struct razer_report razer_chroma_extended_matrix_effect_static(
    unsigned char variable_storage, unsigned char led_id,
 ...
 *
 * Status Trans Packet Proto DataSize Class CMD Args
 * 00     3f    0000   00    06       0f    02  010504002800 | SET LED MATRIX
 * Effect (VARSTR, Backlight, Wave 0x04, Dir 0x00, ? 0x2800) 00     3f    0000
 * 00    06       0f    02  010504012800 | SET LED MATRIX Effect (VARSTR,
 * Backlight, Wave 0x04, Dir 0x01, ? 0x2800)
 */
struct razer_report razer_chroma_extended_matrix_effect_wave(
    unsigned char variable_storage, unsigned char led_id,
 ...
 *
 * Status Trans Packet Proto DataSize Class CMD Args
 * 00     3f    0000   00    06       0f    02  010507000100             | SET
 * LED MATRIX Effect (VARSTR, Backlight, Starlight 0x07, ? 0x00, Speed 0x01,
 * Colours 0x00) 00     3f    0000   00    06       0f    02  010507000200 | SET
 * LED MATRIX Effect (VARSTR, Backlight, Starlight 0x07, ? 0x00, Speed 0x02,
 * Colours 0x00) 00     3f    0000   00    06       0f    02  010507000300 | SET
 * LED MATRIX Effect (VARSTR, Backlight, Starlight 0x07, ? 0x00, Speed 0x03,
 * Colours 0x00) 00     3f    0000   00    09       0f    02  010507000301ff0000
 * | SET LED MATRIX Effect (VARSTR, Backlight, Starlight 0x07, ? 0x00, Speed
 * 0x03, Colours 0x01, RGB 0xFF0000) 00     3f    0000   00    0c       0f    02
 * 010507000302ff000000ff00 | SET LED MATRIX Effect (VARSTR, Backlight,
 * Starlight 0x07, ? 0x00, Speed 0x03, Colours 0x02, RGB 0xFF0000, RGB 0x00FF00)
 */
 ...
 *
 * Status Trans Packet Proto DataSize Class CMD Args
 * 00     3f    0000   00    06       0f    02  010503000000 | SET LED MATRIX
 * Effect (VARSTR, Backlight, Spectrum 0x03, 0x000000)
 */
struct razer_report razer_chroma_extended_matrix_effect_spectrum(
    unsigned char variable_storage, unsigned char led_id) {
  return razer_chroma_extended_matrix_effect_base(0x06, variable_storage,
                                                  led_id, 0x03);
}
 ...
 *
 * Status Trans Packet Proto DataSize Class CMD Args
 * 00     3f    0000   00    09       0f    02  010505000101ffff00 | SET LED
 * MATRIX Effect (VARSTR, Backlight, Reactive 0x05, ? 0x00, Speed 0x01, Colours
 * 0x01, RGB 0xFFFF00) 00     3f    0000   00    09       0f    02
 * 010505000101ff0000 | SET LED MATRIX Effect (VARSTR, Backlight, Reactive 0x05,
 * ? 0x00, Speed 0x02, Colours 0x01, RGB 0xFF0000) 00     3f    0000   00    09
 * 0f    02  010505000301ff0000 | SET LED MATRIX Effect (VARSTR, Backlight,
 * Reactive 0x05, ? 0x00, Speed 0x03, Colours 0x01, RGB 0xFF0000) 00     3f 0000
 * 00    09       0f    02  010505000401ff0000 | SET LED MATRIX Effect (VARSTR,
 * Backlight, Reactive 0x05, ? 0x00, Speed 0x04, Colours 0x01, RGB 0xFF0000)
 */
 ...
 *
 * Status Trans Packet Proto DataSize Class CMD Args
 * 00     3f    0000   00    09       0f    02  01050201000100ff00       | SET
 * LED MATRIX Effect (VARSTR, Backlight, Breathing 0x02, Colours 0x01, ? 0x00,
 * Colours 0x01, RGB 0x00FF00) 00     3f    0000   00    0c       0f    02
 * 01050202000200ff00ff0000 | SET LED MATRIX Effect (VARSTR, Backlight,
 * Breathing 0x02, Colours 0x02, ? 0x00, Colours 0x02, RGB 0x00FF00, RGB
 * 0xFF0000) 00     3f    0000   00    06       0f    02  010502000000 | SET LED
 * MATRIX Effect (VARSTR, Backlight, Breathing 0x02, Colours 0x00, ? 0x0000)
 */
 ...
 *
 * Status Trans Packet Proto DataSize Class CMD Args
 * 00     3f    0000   00    0c       0f    02  000008000000000000000000   |
 * DRAW LED MATRIX Frame
 */
 ...
 */
struct razer_report razer_chroma_extended_matrix_brightness(
    unsigned char variable_storage, unsigned char led_id,
    unsigned char brightness) {
 ...
 */
struct razer_report razer_chroma_extended_matrix_get_brightness(
    unsigned char variable_storage, unsigned char led_id) {
...
struct razer_report razer_chroma_extended_matrix_set_custom_frame(
    unsigned char row_index, unsigned char start_col, unsigned char stop_col,
    unsigned char *rgb_data) {
  return razer_chroma_extended_matrix_set_custom_frame2(
      row_index, start_col, stop_col, rgb_data, 0x47);
}

struct razer_report razer_chroma_extended_matrix_set_custom_frame2(
    unsigned char row_index, unsigned char start_col, unsigned char stop_col,
    unsigned char *rgb_data, size_t packetLength) {
  const size_t row_length = (size_t)(((stop_col + 1) - start_col) * 3);
  // Some devices need a specific packet length, most devices are happy with
  // 0x47 e.g. the Mamba Elite needs a "row_length + 5" packet length
  const size_t data_length =
      (packetLength != 0) ? packetLength : row_length + 5;
 ...
 *
 * Status Trans Packet Proto DataSize Class CMD Args
 *  * 00     3f    0000   00    03       03    0d  010100 | SET Extended Matrix
 * Effect (VARSTORE, LOGO_LED, OFF)
 */
struct razer_report razer_chroma_mouse_extended_matrix_effect_none(
    unsigned char variable_storage, unsigned char led_id) {
  return razer_chroma_mouse_extended_matrix_effect_base(0x03, variable_storage,
                                                        led_id, 0x00);
}
 ...
 *
 * Status Trans Packet Proto DataSize Class CMD Args
 * 00     3f    0000   00    06       03    0d  010106 00ff00 | SET Extended
 * Matrix Effect (VARSTORE, SCROLL_WHEEL, STATIC, RGB)
 */
struct razer_report razer_chroma_mouse_extended_matrix_effect_static(
    unsigned char variable_storage, unsigned char led_id,
    struct razer_rgb *rgb) {
  struct razer_report report = razer_chroma_mouse_extended_matrix_effect_base(
 ...
 *
 * Status Trans Packet Proto DataSize Class CMD Args
 * 00     3f    0000   00    03       03    0d  010104 | SET Extended Matrix
 * Effect (VARSTORE, LOGO_LED, SPECTRUM)
 */
 ...
 *
 * Status Trans Packet Proto DataSize Class CMD Args
 * 00     3f    0000   00    07       03    0d  010102 0300ff00            | SET
 * Extended Matrix Effect (VARSTORE, SCROLL_WHEEL,  REACTIVE, TIME, RGB) 00 3f
 * 0000   00    07       03    0d  010102 0200ff00            | SET Extended
 * Matrix Effect (VARSTORE, SCROLL_WHEEL,  REACTIVE, TIME, RGB) 00     3f 0000
 * 00    07       03    0d  010102 0100ff00            | SET Extended Matrix
 * Effect (VARSTORE, SCROLL_WHEEL,  REACTIVE, TIME, RGB)
 */
 ...
 *
 * Status Trans Packet Proto DataSize Class CMD Args
 * 00     3f    0000   00    0a       03    0d  010103 0100ff00000000      | SET
 * Extended Matrix Effect (VARSTORE, SCROLL_WHEEL, BREATHING, single, RGB,
 * RGB-none) 00     3f    0000   00    0a       03    0d  010103 0200ff00ff0000
 * | SET Extended Matrix Effect (VARSTORE, SCROLL_WHEEL, BREATHING, dual, RGB,
 * RGB) 00     3f    0000   00    0a       03    0d  010103 03000000000000 | SET
 * Extended Matrix Effect (VARSTORE, SCROLL_WHEEL, BREATHING, random, RGB-none,
 * RGB-none)
 */
 ...
 */
/**
 * Toggled whether F1-12 act as F1-12 or if they act as the function options
 * (without Fn pressed)
 *
 ...
 * ? TODO fill this
 */
struct razer_report razer_chroma_misc_set_blade_brightness(
    unsigned char brightness) {
...
/**
 * Set the charging effect, think if I remember correctly, it's either static
 * colour, or "whatever the mouse was last on"
 */
struct razer_report razer_chroma_misc_set_dock_charge_type(
 ...
 * 0x08 =  125Hz
 */
struct razer_report razer_chroma_misc_set_polling_rate(
 ...
 */
struct razer_report razer_chroma_misc_set_dock_brightness(
    unsigned char brightness) {
 ...
 */
struct razer_report razer_chroma_misc_get_dpi_xy(
    unsigned char variable_storage) {
...
/**
 * Set the DPI of the device (Some stupid turd scaled 5600 dpi into a single
 * byte)
 */
...
/**
 * Get the DPI of the device (Some stupid turd scaled 5600 dpi into a single
 * byte)
 */
 ...
 * 0x0C =  5%
 */
struct razer_report razer_chroma_misc_set_low_battery_threshold(
    unsigned char battery_threshold) {
...
struct razer_report razer_chroma_misc_set_orochi2011_led(
    unsigned char led_bitfield) {
