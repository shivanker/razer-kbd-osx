...
//  RazerBlade
//
//  Based on Linux razer-driver kernel drivers:
//  https://github.com/terrycain/razer-drivers/blob/master/driver/razercommon.h
//
 ...
 *
 * Makes a request like normal, this must change a variable in the device as
 * then we tell it give us data and it gives us a report.
 *
 * Supported Devices:
 *   Razer Chroma
 *   Razer Mamba
 ...
 *   Razer Firefly*
 *
 * Request report is the report sent to the device specifing what response we
 * want Response report will get populated with a response
 *
...
IOReturn razer_get_usb_response(IOUSBDeviceInterface **dev, uint report_index,
  ...
  // TODO look to see if index needs to be different for the request and the
  // response
